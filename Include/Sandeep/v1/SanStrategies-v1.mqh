...
// Print("[TIME] : Current: "+ TimeToString(TimeCurrent(), TIME_DATE|TIME_MINUTES)+" GMT: "+ TimeToString(TimeGMT(), TIME_DATE|TIME_MINUTES));
   ...
   Print("[OPEN] :: domSIG: " + util.getSigString(dominantSIG) +" cp120: " + util.getSigString(hSig.cpSlopeCandle120SIG) +" momSIG: "+util.getSigString(ss.momSIG) + " tradeSlopeSig: "+ util.getSigString(ss.tradeSlopeSIG)+" slopeAnalyzerSIG: "+util.getSigString(ss.slopeAnalyzerSIG)+" volatilityMomentum: "+util.getSigString(ss.volatilitySIG)); 
   Print("[FOPEN] c_SIG: " + util.getSigString(hSig.c_SIG)+" Slope30: " + util.getSigString(hSig.simpleSlope_30_SIG)+" slope120: " + util.getSigString(hSig.slopeCandle120SIG) + " fastSIG: " + util.getSigString(hSig.fastSIG)+ " cpScatt: " + util.getSigString(ss.cpScatterSIG) + " sVarSIG: " + util.getSigString(ss.slopeVarSIG) + " candleVol: " + util.getSigString(ss.candleVolSIG));
